		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
	            	...
	            	html += '						<input accept="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel" id="fileImage" type="file" size="30" name="fileselect[]" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = localhostPath + projectName+"/Hplus/plugins/upload/control/images/fileType/";
				...
				}else if(file.name.indexOf(".xls") > 0 || file.name.indexOf(".xlsx") > 0){
					fileImgSrc = fileImgSrc + "xls.png";
				}else if(file.name.indexOf(".ppt") > 0 || file.name.indexOf(".pptx") > 0){
			...
			this.createCorePlug = function(){
				var params = {
					...
					onDelete: function(file, files) {
						...
						// 重新设置统计栏信息
						uploadeFilesNum=uploadeFilesNum-1;
					...
					onSuccess: function(file, response) {
						...
						$("#uploadInf").append("<p>上传文件：" + fileName + " 成功！</p>");
						uploadeFilesNum=uploadeFilesNum+1
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
