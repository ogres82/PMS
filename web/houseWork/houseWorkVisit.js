...
var TableInit = function (){
	...
	oTableInit.Init = function (){
		$('#evnetSendInfo').bootstrapTable({
			url: './../houseWork/evnetVisit.app?method=listEventVist',         //请求后台的URL（*）        //请求后台的URL（*）
...
function show(obj,type){
	if(type==1)
	{
		showHouseWorkVisit(type)	
	}else if(type==2)
	{
		houseWorkVisitHandle(type)
	}
...
//查看
function showHouseWorkVisit(type)
{
	$("#visit_content").attr("disabled",true);
	$("#visit_evaluate").attr("disabled",true);
	$("#visit_from").attr("disabled",true);
	$("#btfile").hide();
	$("#btVisit").hide();

}
//回访
function houseWorkVisitHandle(type)
{
	
	$("#visit_content").attr("disabled",false);
	$("#visit_evaluate").attr("disabled",false);
	$("#visit_from").attr("disabled",false);
	$("#btfile").hide();
	$("#btVisit").show();
}
//人的中文名字显示
