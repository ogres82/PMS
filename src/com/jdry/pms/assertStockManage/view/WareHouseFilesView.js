...
function initFrame() {
	var menu = view.id("dsWarehouse").get("data").toArray();
...
function getMenuHtml(menuData,isFirstLevel){
	 var html = [];
		...
		for (var i = 0; i < menuData.length; i++) {
			...
			html.push('<td><div class="checkbox i-checks sub-check"><input type = "checkbox" name="'+menuData[i]._data.warehouse_id+'" /></div></td>');
			html.push('<td>'+menuData[i]._data.warehouse_code+'</td>');
			html.push('<td>'+menuData[i]._data.warehouse_name+'</td>');
			html.push('<td>'+menuData[i]._data.warehouse_address+'</td>');
			html.push('<td>'+menuData[i]._data.link_man+'</td>');
			html.push('<td>'+menuData[i]._data.link_phone+'</td>');
			html.push('<td>'+menuData[i]._data.other+'</td>');
			html.push('</tr>');
		}
		html.push('</tbody>');
