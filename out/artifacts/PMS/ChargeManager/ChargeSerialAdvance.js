...
var TableInit = function() {
	...
	oTableInit.Init = function() {
		$('#chargeSerial').bootstrapTable({
			url : './../ChargeManager/ChargeSerial.app?method=serialList&charge_type=01&charge_type_opp=true', // 请求后台的URL（*）
	...
	$.post(url, function(data) {
		...
		for (var j = 0; j < list.length; j++) {
			...
			if (code == 'charge_serial_state') {
				...
				$("<option value='" + detailList[1] + "'>" + detailList[2] + "</option>").appendTo("#state_select");
			}
			// 支付方式
