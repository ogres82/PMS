...
var TableInit = function (){
	...
	oTableInit.Init = function (){
		$('#empInfo').bootstrapTable({
			url: './../secmanage/chargerDutyList.app?method=list',         //请求后台的URL（*）
    		...
    		columns: [ 
    		{
                ...
                field: 'shiftTakerId',
                title: '接班主管',
                formatter: userFormate  //该列数据转换，执行dateFormate函数
            },{
                field: 'shiftTakeTime',
                title: '接班时间'
            },{
	...
	function initUserDrop(){
		var url="./../secmanage/chargerDutyList.app?method=initUserDrop";
	    $.post(url,
		      function(data){
				 var list = eval(data);
				 var m = new Map(); 
    ...
    function openSaveButton(){
    	...
    	if(flag){
        	...
        	$.ajax({
                type: "post",
                url: "./../secmanage/chargerDutyList.app?method=save",
                data: addJson,
                dataType: "json",
        		async : false,
                success: function(data)
    	...
    	if(isOpen){
    		...
    		if(type==4){
    			layer.confirm("您确定要删除所选信息吗?",{
    		        ...
    		        var url="./../secmanage/chargerDutyList.app?"+urlmethod;
    				$.post(url,
       						{empId:selections+""},
       	    		        function(data){
       							layer.msg(eval(data),{
				...
				var url="./../secmanage/chargerDutyList.app?method=add";
    		    $.post(url,function(data){
					clearForm();
					var res=eval(data);
					var now=getNowFormatDate(true);
				    $("#recCode").prop("disabled", false);
					...
					$("#shiftGoodsTransfer").val("对讲机__台；充电器__个；备用电池__块；耳机__个； 雨衣__ 件；雨鞋__双，强光灯__个；充电器__个；\n\n签字笔__支；钥匙__把；电脑__台；电暖器__个，车辆__台；灭火器__瓶；其他物品：\n\n物品（损坏）异动原因说明:");
